...
export default function Landloard() {
  ...
  const fetchProperties = useCallback(
    async (refetch?: boolean) => {
      try {
        const propertiesData = await getProperties(program, [], null, refetch);
  ...
  const createProperty = async (e: React.FormEvent) => {
    ...
    try {
      const { txSignature, propertyPda } = await createPropertyTransaction(
        provider,
        program,
        form,
        wallet,
      ...
      toast({
        title: "Success",
        description: "Property created successfully: " + propertyPda.toBase58(),
        variant: "default",
      });
      ...
      if (error instanceof SendTransactionError) {
        ...
        toast({
          title: "Error",
          description: "Failed to create property. Please try again later.",
          variant: "destructive",
        });
        return;
      }
              ...
              <Card key={index}>
                <CardHeader>
                  <CardTitle className="flex justify-between items-center">
                    ...
                    <Badge
                      variant={property.is_closed ? "destructive" : "secondary"}
                    >
                ...
                <CardContent>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    {property.token_price_usdc && (
                      <div className="flex items-center">
                        ...
                        <div>
                          <p className="text-sm font-medium">
                            Price per ${property.token_symbol}
                          </p>
                          <p className="text-lg">
                            ${" "}
                  ...
                  <Button className="w-full mt-8">Manage</Button>
                </CardContent>
              </Card>
            ))
          ) : (
